...
def updateglobalvalues(btnobj):
    ...
    config.cursorobj.execute("SELECT * FROM timetableevents")
    for record in config.cursorobj.fetchall():
...
def addtaskbtnclick(btnobj, frame):
    ...
    insertaskbtn=tk.Button(addeventframe,text='Add Event')
            ...
            if start_time<end_time:               
                ...
                try:
                    ...
                    if int(sNotext) in sNolist:
                        ...
                        config.cursorobj.execute(sqlcommand, (btnobj, json.dumps(detailsdict[btnobj]), btnobj))
...
def deletetaskbtnclick(btnobj, frame):
    ...
    def removeclick():
        ...
        if int(sNotext) not in sNolist:
            ...
            sqlcommand = "UPDATE timetableevents SET btnobject=(%s), details=(%s) WHERE btnobject=(%s)"
            config.cursorobj.execute(sqlcommand, (btnobj, json.dumps(detailsdict[btnobj]), btnobj)) #json.dumps used to serialize the data entered and store in the mysql database
            config.mysqlobj.commit()
